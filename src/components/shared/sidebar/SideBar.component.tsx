...
export const SidebarItem = ({
  ...
  return (
    <Link
      href={link}
      className="flex items-center justify-between hover:text-primary-light mb-5"
    >
      {icon}
      <span>{text}</span>
      <span>{count}</span>
...
const Sidebar = ({
  ...
  }, [selectedColors, selectedBrands, selectedCategories, priceRange]); // intentionally excluding onFiltersChange
  ...
  // Handle color selection (single-select)
  const handleColorChange = (colorName: string) => {
    setSelectedColors((prev) =>
      prev.includes(colorName) ? [] : [colorName]
    );
  };
  ...
  const clearAllFilters = () => {
    ...
    setPriceRange(initialPriceRange || [0, 1000]);
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full space-y-6 transition-all duration-300">
        ...
        <AccordionSection
          title="Hot Deals"
          sectionName="hotDeals"
          show={hotDeals?.length > 0}
        >
